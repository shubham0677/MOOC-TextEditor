...
public class AutoCompleteDictionaryTrie implements  Dictionary, AutoComplete {
     ...
     */@Override
     public List<String> predictCompletions(String prefix, int numCompletions) 
     {
    	 ...
    	 if(numCompletions==0) { return ll; }
         
         char[] stem=prefix.toLowerCase().toCharArray();
         
         TrieNode currNode=root;
         for(int i=0;i<stem.length;i++){
    		 currNode=currNode.getChild(stem[i]);
    		 if(currNode==null) { return ll; }
         ...
         while(!list.isEmpty() && numCompletions>0) {
        	 currNode=list.remove();
        	 if(currNode.endsWord()) {
        		 ll.add(currNode.getText());
        		 numCompletions--;
        	 }
        	 for(char c : currNode.getValidNextCharacters()){
        		 list.add(currNode.getChild(c));
        	 }
         }
