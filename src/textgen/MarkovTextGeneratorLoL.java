	...
	public void train(String sourceText)
	{
		...
		for(int i=1;i<Words.length;i++) {
			for(ListNode w : wordList){
				if( prevWord.equals(w.getWord()) ) {
					exist=true;
					Node=w;
					break;
				}
			}
			if(exist) { Node.addNextWord(Words[i]); }
			else {
		...
		while(i<numWords){
			for(ListNode n : wordList){
				if(currWord.equals(n.getWord())){
					ListNode currNode=n;
					String w=currNode.getRandomNextWord(this.rnGenerator);
					output+=w+" ";
					currWord=w;
					break;
				}
...
class ListNode
{
	...
	public String getRandomNextWord(Random generator)
	{
		...
		int index=nextWords.size();
		int i=generator.nextInt(index);
